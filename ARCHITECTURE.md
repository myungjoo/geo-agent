...
│                                                                 │
│  에이전트 실행 시 로드 순서:                                      │
│   1. workspace/prompt-configs/{id}.json 존재 → 사용                 │
...
| **프롬프트 저장** | JSON 파일 (`workspace/prompt-configs/`) | 에이전트 시스템 프롬프트 (workspace fallback → default) |
...
> **구현 현황**: Core 라이브러리(`oauth-manager.ts`)에 URL 생성·토큰 교환·갱신·취소 로직 구현 완료. Dashboard `/api/auth/callback` HTTP 엔드포인트는 미등록 → end-to-end 플로우 미동작.
...
[대시보드: /settings/llm-providers]
     ...
     │
     ├─ OAuth 흐름 시작:
     │   ├─ localhost:3000/api/auth/callback 으로 리다이렉트 설정
     │   ├─ Google 로그인 + 권한 동의 화면
...
| **Agent Prompts** | JSON 파일 (`workspace/prompt-configs/`) | PostgreSQL |
