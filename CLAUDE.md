...
### Hardcoded 유지 (순수 기계적 작업)
- `parseRobotsTxt()` — 표준 규격 기반 파싱
- `computeJsDependencyRatio()` — script 카운트/비율 계산

### 하이브리드 (정적 수집 → LLM 판정)
- `extractSchemaCoverage()` — JSON-LD 존재 확인(코드) + 품질 판정(LLM)
- `analyzeJsDependency()` — script 메트릭 수집(코드) + LLM 접근 차단 여부 판정(LLM)
- `computeContentAnalysis()` — word_count/content_density(코드) + readability_level(LLM)
