...
| A-3 | **Probe 프롬프트 사용자 커스터마이징** | 중간 | Probes: `PromptConfigManager` 연결 완료 (프로브 8종 + judgment 2종). 나머지 에이전트(Strategy/Optimization/Validation)는 미연결 | `probes/synthetic-probes.ts`, `prompts/prompt-config-manager.ts` |
...
**A-3 남은 항목**: Strategy/Optimization/Validation Agent → `PromptConfigManager` 연결 (현재 인라인 상수 직접 사용 중)
...
- [#77](../../issues/77) — 프롬프트 중앙 집중 구조 (WIP)
- [#72](../../issues/72) — LLM Provider 분리
- [#60](../../issues/60) — Token 비용 추적
- [#55](../../issues/55) — 봇 정책-점수 모순 버그
- [#54](../../issues/54) — 점수 근거 설명
- [#50](../../issues/50) — Agent 지식 보강
- [#45](../../issues/45) — 5-Phase 고도화 계획

### 개선 로드맵 요약

상세 내용은 GitHub Issue #4, #45 + ARCHITECTURE.md 참조.
