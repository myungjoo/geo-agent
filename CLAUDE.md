...
- packages/core/src/agents/geo-eval-extractor.ts — 신규 모듈
  ...
  - extractGeoEvaluationData(): 전체 통합 → GeoEvaluationData (봇 정책, 스키마, 클레임, JS, 제품, 개선안)
...
#### Evaluation 탭 samsung dashboard 수준 확장 ✅
- dashboard.html Evaluation 탭 8개 섹션:
  1. 📊 GEO Score 게이지 (before/after + delta)
  2. 📈 7-Dimension GEO Scores 진행바
  3. 🤖 AI Bot Access Policy 테이블 (봇별 상태 + llms.txt 카드)
  4. 🏗️ Schema.org Coverage Matrix (12타입 × present/quality)
  5. 📦 Product Information Extracted (페이지별 제품명/가격/스펙/평점)
  6. 💬 Marketing Claims & Verifiability (클레임 × 출처 × 검증성)
  7. ⚙️ JS Dependency Analysis (스크립트 수 + 프레임워크 + 의존도)
  8. 🎯 Improvement Roadmap (Sprint 1/2/3별 그룹, impact 별점, difficulty 레이블)
...
#### 테스트 (vitest) — 1255+ tests ✅
- Phase 5까지: 1182 tests
- Phase 6 추가: viz-spec 51 + stage-execution 15 + pipeline API 6 + regression 1 = 73 tests
