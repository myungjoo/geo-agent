...
- `safeLLMCall()` — 에러 시 graceful degradation (인증 에러는 re-throw → 파이프라인 중단)
...
| A-7 | **Machine Readability Grade 종합 판정** | 낮음 | overall_score 기반 (JS+시맨틱+크롤러 미종합) | `analysis/analysis-agent.ts` |
...
**A-4 구현 방향**: TargetProfile.competitors URL 크롤 + 채점 → 차원별 격차 계산
**A-5 구현 방향**: `extractProductInfo()` 결과 → `InfoRecognitionItem` 변환 + Dashboard 검토 UI
