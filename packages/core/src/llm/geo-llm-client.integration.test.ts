...
function findWorkspaceWithApiKey(): string | null {
	const candidates = [
		path.join(process.env.HOME ?? "", ".geo-agent"),
		"./run",
		".",
	];
...
describe("GeoLLMClient — real API integration", () => {
	it.skipIf(!hasApiKey)("should complete a simple prompt via pi-ai", async () => {
	...
	it.skipIf(!hasApiKey)("should handle json_mode correctly", async () => {
		const client = new GeoLLMClient(workspaceDir!);
	...
	it.skipIf(!hasApiKey)("should populate all LLMResponse fields", async () => {
		const client = new GeoLLMClient(workspaceDir!);
