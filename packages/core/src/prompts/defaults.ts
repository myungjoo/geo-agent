...
export const DEFAULT_PROMPTS: Record<string, Omit<AgentPromptConfig, "last_modified">> = {
	orchestrator: {
		...
		context_slots: [
			{ slot_name: "{{TARGET_PROFILE}}", description: "TargetProfile JSON", source: "TargetProfile", required: true },
			{ slot_name: "{{PIPELINE_STATE}}", description: "파이프라인 상태", source: "PipelineState", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
		],
	...
	analysis: {
		...
		context_slots: [
			{ slot_name: "{{TARGET_PROFILE}}", description: "TargetProfile JSON", source: "TargetProfile", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
			{ slot_name: "{{ANALYSIS_HISTORY}}", description: "이전 분석 결과 요약", source: "AnalysisReport[]", required: false },
		],
		model_preference: null,
		temperature: 0.3,
		is_customized: false,
	...
	strategy: {
		...
		context_slots: [
			{ slot_name: "{{TARGET_PROFILE}}", description: "TargetProfile JSON", source: "TargetProfile", required: true },
			{ slot_name: "{{ANALYSIS_REPORT}}", description: "분석 보고서", source: "AnalysisReport", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
		],
		model_preference: null,
		temperature: 0.5,
	...
	optimization: {
		...
		context_slots: [
			{ slot_name: "{{TARGET_PROFILE}}", description: "TargetProfile JSON", source: "TargetProfile", required: true },
			{ slot_name: "{{OPTIMIZATION_PLAN}}", description: "최적화 계획", source: "OptimizationPlan", required: true },
			{ slot_name: "{{CURRENT_SNAPSHOT}}", description: "현재 페이지 스냅샷", source: "ContentSnapshot", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
		],
		model_preference: null,
		temperature: 0.2,
	...
	validation: {
		...
		context_slots: [
			{ slot_name: "{{TARGET_PROFILE}}", description: "TargetProfile JSON", source: "TargetProfile", required: true },
			{ slot_name: "{{CHANGE_RECORDS}}", description: "검증 대상 변경 기록", source: "ChangeRecord[]", required: true },
			{ slot_name: "{{SCORE_BEFORE}}", description: "변경 전 GEO 점수", source: "GeoScore", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
		],
		model_preference: null,
		temperature: 0.1,
	...
	monitoring: {
		...
		context_slots: [
			{ slot_name: "{{ACTIVE_TARGETS}}", description: "모니터링 대상 타겟 목록", source: "TargetProfile[]", required: true },
			{ slot_name: "{{AVAILABLE_TOOLS}}", description: "사용 가능 도구 목록", source: "ToolRegistry", required: true },
		],
		model_preference: null,
		temperature: 0.2,
